...
@click.option('-r',
              ...
              type=int,
              default=15,
              help='Playback rate (images per second)')
def main(indir, output, rate):
    images = [
        img for img in os.listdir(indir)
    ...
    if output is None:
        output = os.path.abspath(indir) + '.mp4'
    ...
    for image in images:
        ...
        image = cv2.putText(image, watermark, org, font, fontScale, color,
